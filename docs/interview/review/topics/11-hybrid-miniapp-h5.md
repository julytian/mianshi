...
开场一分钟报约束：目标端、包体/审核、首开预算。三分钟小程序：双线程、`setData`、分包决策、登录隐私。四分钟 Hybrid：Bridge 协议字段、来源校验、隔离与 iOS 17+ store 边界、热更新供应链一句话。四分钟 H5：安全区协议、弱网幂等、返回栈状态机。最后用自己的主包体积或桥失败率收口。明确说：uni-app 降低表达成本，不消灭测试矩阵。被追问插件细节时，回到能力协商与真机契约，不现场背第三方文档版本号。
