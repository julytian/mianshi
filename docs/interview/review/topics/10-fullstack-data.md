...
前三分钟立边界：NestJS 11 / Prisma 锁定版本、三层职责、错误码是产品契约。中间六分钟只深挖一条主线——要么「下单短事务 + 幂等对账」，要么「列表 N+1 + 索引 + 游标分页」，配自己的 Trace / p95 数字。后六分钟讲迁移 expand-and-contract 或一次联调排障方法论，并主动声明：不是来拼 JVM 调优或分布式事务八股。收口句：「类型安全不代替约束，防抖不代替幂等，deploy 不代替评审。」
