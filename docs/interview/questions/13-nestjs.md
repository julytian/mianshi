...
全部集中会让模块变成隐式服务定位器：任何业务都能越过边界依赖内部实现，循环依赖、初始化副作用和测试装配随规模放大。更稳妥的是按业务能力声明 Provider，只导出稳定门面或 token；真正无业务状态、跨域通用且依赖很少的能力，才进入 SharedModule。

**2. Controller 可以直接调用 Repository 吗？**

技术上可以注入，但通常只适合规则极少的简单 CRUD。只要涉及事务、权限外的业务校验、领域状态转换或多个数据源，Controller 直连 Repository 就会把协议层与持久化耦合。应由应用服务承接用例和事务，Controller 只转换输入、调用用例并映射输出。

**3. 一个模块过大时用什么信号拆分？**

看变化原因而非文件数量：同一模块出现多套业务语言、不同数据所有权、互不相关的发布节奏，或跨团队修改频繁冲突，都是拆分信号。先在单体内提取子模块和公开门面，建立单向依赖与契约测试；只有独立扩缩容、故障隔离收益明确时再考虑拆成服务。
...
**1. `exports` 导出类与导出 token 有何区别？**

导出类时，类本身既是实现也是 DI token，消费方按该类注入；导出字符串或 `Symbol` token 时，消费方依赖的是抽象契约，可在模块内部用 `useClass`、`useFactory` 等替换实现。两者都必须先在本模块注册，不能仅写进 `exports` 就凭空创建 Provider。

**2. `useExisting` 和 `useClass` 是否共享实例？**

`useExisting` 为已经注册的 Provider 建立别名，两个 token 指向同一个容器实例，生命周期与原 Provider 一致。`useClass` 会让容器针对该定义实例化指定类；即使类相同，也可能形成另一份实例。需要共享连接、缓存或状态时用别名，替换实现时才用 `useClass`。
...
可以。Nest 会等待异步工厂解析后再完成依赖装配，`forRootAsync` 常用它读取配置或创建客户端。但工厂位于启动关键路径，失败会阻止应用就绪，因此必须设置连接超时、明确重试上限并输出可诊断错误；创建的连接还要由生命周期钩子负责释放。

**2. 测试里怎样覆盖 custom provider？**

使用 `Test.createTestingModule()` 按生产模块装配，再在 `compile()` 前调用 `overrideProvider(TOKEN).useValue()`、`useClass()` 或 `useFactory()`。覆盖的必须是消费方实际注入的同一个 token。单测可用内存替身，集成测试则尽量保留真实装配，以发现 imports、exports 和 scope 错误。

**3. 为什么接口不能直接作为注入 token？**

TypeScript interface 只参与静态类型检查，编译成 JavaScript 后会被擦除，运行时容器拿不到可比较的键。应使用类、集中定义的字符串或 `Symbol` 作为 token，并通过 `@Inject(TOKEN)` 注入接口类型。跨模块优先 `Symbol`，同时从唯一契约文件导出，避免重复创建。
...
**1. request scope 如何影响 Controller？**

当 Controller 直接或间接依赖 request-scoped Provider 时，默认单例实例无法固定这条依赖，Controller 也会随每个请求重新创建，scope 沿依赖链向上扩散。它会增加实例化、垃圾回收和内存分配成本，应统计实例数与 P95/P99，并优先把用户、租户等上下文显式传参。

**2. GraphQL 为什么注入 `CONTEXT`？**

HTTP 的 `REQUEST` token 代表底层请求对象，而 GraphQL 一次执行使用的是由适配器构造的 GraphQL context，其中可能包含 request、用户和 DataLoader。Resolver 或 request-scoped Provider 应注入 `CONTEXT`，再按项目约定取值；硬依赖 HTTP Request 会让订阅、测试和非 HTTP transport 失真。

**3. durable provider 的复用边界如何证明安全？**

必须证明同一分组内可共享的只有租户级不可变配置或安全缓存，不包含用户身份、请求参数、事务上下文等单请求状态。为 `ContextIdStrategy` 做隔离与并发测试，验证不同租户绝不复用；再通过压测比较分配与延迟收益，并为租户数量和缓存回收设置上限。
...
`forRoot` 通常注册全局或连接级配置与核心客户端，一般在应用根部调用一次；`forFeature` 基于既有客户端登记某个业务模块需要的模型、仓储或局部配置，可被多个特性模块调用。两者只是库约定，不是 Nest 强制语义，库作者应明确重复调用、导出项和资源归属。

**2. 为什么异步工厂不能随意发起无超时网络请求？**

异步工厂处于依赖图初始化阶段，Promise 不结束会让应用永远无法进入 ready，编排器只看到启动失败或反复重启。远程配置、密钥和连接创建都应有超时、有限重试与清晰降级策略；若能力非启动必需，可延迟初始化，但仍要处理并发、失败缓存和关闭。

**3. 动态模块重复导入时实例一定共享吗？**

不一定。是否共享取决于动态模块返回的 module identity、Provider 注册位置、scope、是否全局以及每次配置生成的 token。重复调用工厂可能建立多份客户端或覆盖配置，不能仅凭类名推断。库应记录重复导入语义，应用通过装配测试和实例计数验证，连接型资源最好集中所有权。
...
**1. TypeScript interface 为什么不能作为 token？**

interface 在编译后不存在，Nest 运行时无法用它在容器中查找 Provider；反射得到的也不会是接口契约。应定义唯一的类、字符串或 `Symbol` token，并使用 `@Inject()` 明确关联接口类型与实现。token 要从共享契约模块导出，避免两个文件各自创建同名但不同的 `Symbol`。

**2. 类级和方法级权限如何覆盖？**

先定义产品语义：方法级是替换类默认权限，还是与类权限合并。替换可用 `Reflector.getAllAndOverride()` 按 handler、class 顺序取首个值；合并可用 `getAllAndMerge()`，再去重并按 AND/OR 规则判定。不要依赖偶然顺序，需用无元数据、仅类级和两级同时存在的测试锁定。

**3. Decorator 组合如何测试？**

分别验证声明与执行：单元测试读取组合 Decorator 写入的 metadata，确认 key、值及类级和方法级目标；Guard 测试用 `ExecutionContext` 验证解释逻辑。再做最小 E2E，把组合 Decorator 挂到真实路由，覆盖允许、拒绝和覆盖顺序，避免只测函数调用却漏掉框架绑定。
...
文件循环通常在模块加载阶段出现 `undefined`、初始化前访问或 bundler 循环告警，与 Nest 是否创建容器无关；容器循环则常报无法解析构造依赖，并指出参数索引或模块上下文。先检查 import 图和 barrel file，再核对双方 Provider 注册；移动 token 后仍报 DI 错误，才处理容器关系。

**2. 事件解耦会引入哪些一致性成本？**

同步调用改为事件后，调用方通常只能确认事件已记录，不能立即确认下游业务完成，于是出现最终一致窗口、重复投递、乱序、失败重试和补偿问题。需要 outbox、幂等键、状态查询、死信与对账，并在界面表达「处理中」。若业务要求强同步原子结果，事件未必合适。

**3. 使用 `ModuleRef.get()` 有哪些测试代价？**

它把构造函数中显式可见的依赖改成运行时查找，静态分析和测试替身注入都更困难；漏注册往往到执行特定分支才报错。测试必须构造真实模块上下文、注册正确 token，并覆盖查找失败与 scope 选项。仅为绕过循环而大量使用，会把架构问题变成隐式依赖。
...
**1. `(body, params, query)` 的 Pipe 顺序？**

Nest 在同一层 Pipe 中按方法参数从最后一个向第一个处理，因此该签名先处理 `query`，再处理 `params`，最后处理 `body`；然后才进入下一层 Pipe。层级顺序是 global、controller、route、parameter，同一层多个 Pipe 按声明顺序执行。不要把参数书写顺序误认为执行顺序。

**2. Pipe 抛错后哪些层能观察到？**

Pipe 位于 Interceptor 入站调用 `next.handle()` 之后、handler 之前，因此异常会跳过 Controller / Provider 和后续正常流程。包裹该流的 Interceptor 可在 `catchError`、`tap` 错误回调或 `finalize` 中观察；若未被恢复成正常值，最终进入最近的 Exception Filter，Middleware 的普通后置代码则未必执行。

**3. route Filter 捕获后 global Filter 是否还执行？**

默认不会。Filter 从路由级、Controller 级到全局级向外查找，一旦最近的 Filter 完成响应，框架不会像 Interceptor 链那样自动继续调用外层 Filter。若路由 Filter 只想处理特定异常，应限制 `@Catch()` 类型；不要捕获所有异常后期待全局 Filter 再统一包装。
...
Middleware 更早执行，适合解析 Cookie、建立 trace 或挂载通用身份线索，但通常拿不到精确 handler metadata。Guard 基于 `ExecutionContext` 和 `Reflector`，能结合路由权限决定是否放行。常见做法是底层解析可复用，最终认证授权在 Guard 完成，并让失败统一进入异常契约。

**2. 缓存放 Interceptor 有什么限制？**

Interceptor 易按路由短路并复用响应，但 cache key 必须包含用户、租户、权限、版本、参数和内容协商等隔离维度；写操作失效、流式响应、文件、SSE 与 `@Res()` 也不能机械包装。缓存只覆盖 handler 执行结果，不能替代数据库一致性设计，还要防止错误响应被缓存。

**3. 业务异常应该在哪里定义、在哪里映射？**

领域或应用层定义与 HTTP 无关的稳定错误类型和业务码，例如余额不足、状态冲突；用例在规则失败处抛出。接口层的全局 Filter 再映射为 HTTP 状态、公开 message、details 和 traceId。这样同一用例可用于 HTTP、RPC 或任务消费者，同时避免数据库异常直接泄露给客户端。
...
`whitelist: true` 会移除 DTO 中没有校验装饰器声明的输入属性，继续处理剩余数据；再开启 `forbidNonWhitelisted: true` 时，发现额外属性会直接抛出校验异常。普通兼容接口可选择剥离，高风险写接口更适合拒绝，并把字段路径映射为稳定错误结构。

**2. 为什么 `id: number` 不代表收到的一定是数字？**

类型标注只在编译期约束服务端代码，HTTP path、query 和 JSON 都是运行时不可信输入，路由参数最初通常还是字符串。必须用 `ParseIntPipe` 或 DTO 配合 `ValidationPipe` 转换并校验整数范围。还要覆盖空字符串、`NaN`、小数、超大数和前导符号，不能只依赖断言。

**3. Partial DTO 如何避免更新接口误清空字段？**

先区分字段缺失、显式 `null` 与空字符串的契约：缺失表示不更新，`null` 只有业务允许清空时才接受。使用映射类型生成可选字段后仍要保留每个字段的运行时校验；组装更新数据时只挑选已出现且通过校验的属性，不用默认值覆盖数据库现值，并为清空动作写专门测试。
...
**1. `catchError` 后 Filter 何时还能收到异常？**

只有 `catchError` 返回 `throwError()`、抛出新异常，或其恢复流再次报错时，未捕获异常才会继续进入 Filter。若返回 `of(value)`，错误已转换为正常响应，Filter 不会触发，甚至可能得到 HTTP 200。Interceptor 应只做必要的错误翻译并保持失败语义，协议映射集中交给 Filter。

**2. RxJS 超时后数据库写入为何可能继续？**

`timeout` 取消的是 Observable 订阅链，不会自动撤销已经启动的 Promise、数据库命令或远程请求。驱动若没有收到取消信号，服务器端 SQL 仍会执行并可能提交，所以客户端看到 504 不代表副作用停止。写操作必须幂等，并结合驱动取消、statement timeout 和结果隔离控制风险。

**3. 如何把 deadline 传到底层？**

入口根据网关剩余时间生成绝对 deadline 或 `AbortSignal`，应用服务继续显式传给 HTTP 客户端、数据库适配器和队列调用；每层用剩余预算设置更短超时，而不是重新获得完整时长。底层不支持取消时，要记录超时后仍运行的任务，并通过幂等键、状态机和补偿隔离迟到结果。
...
401 表示请求缺少可接受的认证凭证，例如 token 缺失、过期或签名无效，客户端可进入登录或受控刷新流程；403 表示身份已经确认，但没有执行该动作的权限。不要用 403 隐藏所有认证问题，也不要对 401 无限刷新。具体业务分支还应结合稳定错误码。

**2. 校验错误如何表达字段级信息？**

返回稳定结构，例如 `code`、`message`、`details.fields[]` 与 `traceId`；每个字段项包含规范化路径、机器可读规则码和必要参数。不要把 `class-validator` 的英文 constraint 文本直接作为长期契约，也不要回显敏感原值。嵌套数组路径应统一表示，并用契约测试锁定前后端解析。

**3. 前端是否应该根据 `message` 写业务分支？**

不应该。`message` 面向人类，可能因本地化、文案优化或安全脱敏而变化；业务判断应依赖稳定 `code`、HTTP 状态和结构化字段。前端可把未知 code 降级为通用提示并记录 traceId。服务端变更错误码要按 API 契约管理，不能把数据库原始错误当作机器码。
...
**1. plain object 为什么可能绕过预期序列化？**

`class-transformer` 的装饰器元数据绑定在类及其属性上，ORM 投影或手写字面量只是 plain object，没有目标类实例信息时，`@Exclude()`、分组和转换规则可能不生效。敏感字段应在查询投影和输出 DTO 两层排除，并在 E2E 断言真实 JSON，不能只看 TypeScript 返回类型。

**2. 版本放 URI 还是 Header？**

URI 版本直观，便于浏览器、网关、缓存、日志和手工排障，适合多数公开 API；Header 或 Media Type 能保持 URL 稳定，但客户端配置、代理缓存键和可观测性更复杂。选择取决于消费方与基础设施能力。无论哪种，都要定义默认版本、弃用通知和并存窗口。

**3. 如何在 CI 发现 breaking change？**

由运行时 DTO 和路由生成 OpenAPI，与基线版本做语义 diff，阻止删除路径、收紧输入、移除枚举值或改变必填性等破坏性变更；再生成客户端做类型编译，并对关键消费者运行契约测试。Schema diff 不能覆盖所有行为变化，所以还要用 E2E 验证状态码、错误码和序列化。
...
access JWT 可在本地验签，但只要需要登出撤销、设备列表、refresh rotation、权限即时变更或重放检测，服务端仍要维护会话族、版本或黑名单状态。真正的取舍是哪些状态放 token、哪些状态放服务端，以及接受多长的一致性窗口，不能把 JWT 等同于无需存储和治理。

**2. Cookie 鉴权如何防 CSRF？**

先设置 `Secure`、`HttpOnly` 和合适 `SameSite`，但不能把 SameSite 当唯一边界。状态变更接口还应校验 `Origin` / `Referer`，并使用同步 token，或与会话绑定且 HMAC 签名的 double-submit token。CORS 不是 CSRF 防护；服务端还要拒绝简单表单可伪造的敏感请求格式。

**3. 用户改密码后怎样让旧会话失效？**

在事务中更新密码哈希并提升用户凭证版本，或撤销该用户全部 refresh 会话族；后续刷新检查版本后拒绝旧 token。短期 access token 可能在过期前仍有效，可按风险缩短 TTL，或对高风险操作在线检查版本。还应清理设备会话、记录审计并通知用户异常登录。
...
两者携带同一旧 refresh token 时，应由数据库事务、唯一约束或 compare-and-set 保证只有首个请求完成版本推进；另一个看到旧版本后失败。若产品允许极短重叠窗口，必须返回同一旋转结果或限制一次受控复用，不能各自签发不同后继 token，否则会破坏重放检测。

**2. 多设备登出如何做？**

每次登录建立独立 session / token family，记录用户、设备标识、版本、哈希和撤销时间。单设备登出只撤销对应会话；「退出所有设备」在事务中撤销该用户全部会话，并可提升全局凭证版本。客户端删 Cookie 只是清理本地状态，服务端撤销才是安全边界。

**3. token family 被重放后如何告警？**

检测到已旋转或撤销 token 再次使用时，原子撤销整个 family，并记录用户、session、时间、IP、设备摘要、traceId 与原因，严禁记录完整 token。将事件发送到安全监控，按用户和来源聚合去重、分级告警；同时通知用户重新登录，并保留审计链用于调查。
...
前端菜单和按钮只是体验控制，代码、网络请求和客户端状态都可被用户修改，不能构成安全边界。后端必须在每个受保护动作上由 Guard 或应用服务校验身份、权限和资源条件。前端可以复用同一权限 key 展示界面，但最终 401 / 403 与业务拒绝以服务端为准。

**2. 资源属性要查询数据库时 Guard 如何避免重复查询？**

可在请求内由一个授权查询加载最小投影，把已验证资源或授权上下文挂到受控 request context，后续 handler 复用；也可让应用服务同时完成加载和授权，避免 Guard 与用例各查一次。不要使用跨请求全局可变缓存。若采用 request scope 或 DataLoader，要测隔离和实例成本。

**3. 超级管理员如何避免散落硬编码？**

把超级权限作为策略引擎中的显式规则或角色能力，由统一授权服务解释，而不是各处写 `role === 'admin'`。策略需要审计、最小权限和高风险操作的额外约束，必要时仍受租户与合规边界限制。用权限矩阵和契约测试覆盖普通角色、管理员及禁止绕过的动作。
...
**1. Prisma 类型安全覆盖不了哪些运行时问题？**

生成类型不能保证行一定存在、唯一约束不冲突、并发更新无竞态、事务不死锁，也不能代替输入校验、权限和数据库连接可用性。Prisma 7 的具体客户端生成、适配器与事务能力应以项目锁定版本为准；运行时仍需约束、事务、错误映射、超时和真实数据库测试。

**2. 何时直接写 SQL？**

当复杂报表、窗口函数、数据库特有能力、精细锁语义或执行计划无法由 ORM 清晰表达，且显式 SQL 能显著改善可读性或性能时可以使用。SQL 必须参数化，纳入事务上下文、类型映射、迁移与测试，并观测执行计划。不要为了「纯 ORM」制造大量往返，也别随意拼接输入。

**3. 生产迁移为什么不能只靠 auto sync？**

自动同步通常只比较当前模型与数据库，难以表达数据回填、分阶段兼容、锁影响、回滚和多实例滚动发布顺序，破坏性变更还可能直接丢数据。生产应使用可审查、可版本化的迁移，先扩展再迁移数据再收缩，并在副本数据上评估时长、锁和恢复方案。
...
多个记录可能具有相同时间戳，数据库对这些并列行没有稳定顺序；翻页期间插入或删除还会造成重复和遗漏。应追加唯一且不可变的 tie-breaker，例如 `createdAt DESC, id DESC`，cursor 同时编码两者，查询使用对应比较条件，并建立匹配排序方向的复合索引。

**2. 筛选条件改变后旧 cursor 还能用吗？**

通常不能直接复用，因为 cursor 只在原排序与筛选集合中定义位置。应把筛选摘要、排序版本和方向写入签名 cursor，服务端解码后与当前请求比较；不一致就返回明确错误并要求从首页开始。不要接受客户端自行拼出的游标字段，以免越权或产生不可预测查询。

**3. 后台必须跳到第 100 页怎么办？**

若数据规模可控且产品确需随机跳页，可以保留 offset 分页，并通过过滤条件、最大页深和匹配索引限制成本；深页可先查索引上的 ID 再回表。也可提供按日期或关键字定位、导出任务替代任意跳页。应测真实执行计划，而不是强行用 cursor 模拟页码。
...
同时观察池的活跃、空闲、等待队列、获取连接耗时和超时，以及数据库端实际活跃会话、CPU、锁等待和查询时长。若获取连接时间陡增而已执行 SQL 仍快，且池长期满载，更像池耗尽；若连接可得但 SQL 延迟上升，则继续查慢查询、锁和执行计划。

**2. ORM 事务上下文如何避免丢失？**

事务 callback 内只使用框架传入的 transaction client 或 EntityManager，并把它显式传给仓储；不要回退到全局 Prisma Client 或默认 Repository。TypeORM 可创建绑定该 manager 的仓储，Prisma 交互式事务应使用 `tx`。用失败回滚集成测试断言所有写入都处于同一连接和事务。

**3. outbox 何时可能重复投递？**

发布器把消息发送给 broker 后、尚未来得及把 outbox 标记为已发布就崩溃，重启扫描会再次发送；broker 确认丢失也会产生同样结果。因此 outbox 解决不漏记，不承诺 exactly-once。事件需携带稳定 eventId，消费者以唯一约束或幂等记录去重，并保留可重试状态。
...
删除后到数据库提交前，另一个读请求可能回源读到旧值并重新写入缓存；随后写库完成，旧缓存却继续存在，形成不一致。常见做法是先提交数据库再删缓存，并结合重试、消息驱动失效或短 TTL。高一致场景还要使用版本号，不能把延迟双删当绝对保证。

**2. 权限变化后用户缓存如何失效？**

权限缓存 key 应包含用户、租户与权限版本。角色或授权关系变更时，在同一业务流程提升版本并删除相关 key，或发布可靠失效事件；鉴权读取发现版本不匹配就回源。TTL 只作为兜底，不能承担紧急撤权。高风险接口可在线检查最新权限或会话版本。

**3. Redis 故障时系统应如何降级？**

先按缓存用途分级：普通查询缓存可短路回源数据库，但要限并发、防止击穿并保护连接池；Session、限流或分布式锁若依赖 Redis，则不能假装安全，应拒绝相关高风险操作或切换经演练的备用方案。设置短超时和熔断，避免每个请求长时间等待并放大故障。
...
消息可能在消费成功后、确认到达 broker 前发生崩溃，于是被再次投递；网络重试和生产者重发也会制造重复。即使 broker 提供特定 exactly-once 能力，也未必覆盖数据库和外部 API。消费者应以业务键或 eventId 建唯一约束，原子记录结果，并让重复执行返回既有状态。

**2. 分布式锁过期但任务未结束怎么办？**

另一个实例可能获得同一锁并并发执行，因此锁必须有足够租期、续租心跳和所有权校验，释放时只删除自己的 token。更关键的是使用 fencing token：每次获得递增版本，下游拒绝旧持有者写入。任务本身仍需幂等和状态机，不能把租约锁视为永久互斥证明。
...
**1. 数据库不可用时 live 和 ready 各返回什么？**

通常进程与事件循环仍健康时 liveness 继续成功，避免数据库故障触发所有 Pod 重启风暴；readiness 失败，使实例停止接收新流量。若服务有不依赖数据库的能力，可设计分级 ready，但路由治理必须匹配。检查要短超时、低成本，并通过持续阈值避免瞬时抖动摘流。

**2. traceId 如何跨消息传递？**

把 W3C `traceparent`、必要的 `tracestate` 和业务 correlation ID 放进消息 envelope 或 transport header；生产端创建 producer span，消费端提取上下文并创建 consumer span。重试和死信要保留原关联，同时生成新的处理 span。不得把 token、Cookie 等敏感请求头整体复制进消息。

**3. 如何避免高基数指标？**

指标 label 只使用有界维度，如 service、route 模板、方法、状态类别和错误类型，不放 userId、订单号、原始 URL、traceId 或异常文本。个体定位交给日志和 trace，通过 exemplar 关联。上线前估算时间序列数量，为动态标签设白名单、聚合与监控预算。
...
优先读取可信的 `Retry-After` 或服务端限流重置时间，在上限内指数退避并加入随机抖动，避免所有客户端同时重试。非幂等请求不能自动盲重放；界面应提示等待并防重复点击。客户端退避只是体验优化，服务端仍要按身份与接口成本执行限流和容量保护。

**2. 幂等 key 保存多久？**

保存窗口应覆盖客户端可能重试、网关超时和业务副作用完成的最长合理周期。支付、下单常需数小时到数天，并受对账与合规要求影响；过短会让迟到重试重复执行，永久保存又增加成本。记录应包含调用方、操作、请求摘要、状态和结果，并按分区或 TTL 安全清理。

**3. CORS 能否阻止服务端到服务端攻击？**

不能。CORS 是浏览器对前端脚本读取跨源响应的约束，curl、恶意服务端和 SSRF 客户端不受它限制，某些简单请求甚至已发送后才阻止读取。服务端仍需认证授权、输入校验、网络访问控制、限流与 CSRF 防护；严格 allowlist 只减少浏览器侧暴露面。
...
测试事务可能与应用代码不在同一连接，应用内部自行提交、开启新事务或异步任务后，外层回滚无法清理；数据库序列、DDL、消息队列和外部副作用也通常不随事务回滚。需验证真实连接边界，并对这类资源使用独立 Schema、唯一测试命名、显式清理或容器隔离。

**2. 怎样测试 request-scoped Provider？**

用 `ContextIdFactory.create()` 建立独立 context id，通过 `moduleRef.registerRequestByContextId()` 绑定模拟请求，再调用 `moduleRef.resolve(TOKEN, contextId)`。断言同一 context 内依赖复用、不同 context 实例隔离，并覆盖其 Controller 传播。最后用 E2E 验证真实 HTTP 请求和上下文 token。

**3. E2E 为何必须验证全局 Pipe 和 Filter？**

单元测试直接调用 Controller 会绕过 bootstrap 中注册的 `ValidationPipe`、序列化、Guard 和全局 Filter，因此即使方法测试通过，真实接口仍可能接受非法输入或泄露异常。E2E 应复用生产 bootstrap，断言状态码、错误 envelope、字段剥离和 traceId，避免测试应用与生产装配漂移。
...
mock 通常不会模拟真实连接、隔离级别、锁、唯一约束、死锁和回滚语义，也无法发现某个仓储误用了事务外客户端。它适合验证业务分支，不足以证明原子性。事务关键路径必须连接真实数据库，故意让中途步骤失败，并断言所有写入、约束和并发结果符合预期。

**2. 如何验证 Interceptor 没吞异常？**

让 `next.handle()` 返回 `throwError()`，订阅 Interceptor 返回流并断言仍以预期异常结束，而不是发出普通值；同时验证 `finalize` 执行且响应映射未运行。再用 E2E 触发真实 handler 异常，确认全局 Filter 收到并输出非 2xx 的稳定错误契约。

**3. 并发幂等测试怎样避免假阳性？**

使用真实数据库和多个独立连接，以屏障让请求尽量同时进入临界区，所有请求使用同一幂等 key 与摘要。断言业务副作用只发生一次、其余响应遵循约定且结果一致；再用不同摘要确认返回冲突。重复运行并记录时序，不能用单线程 mock Promise 冒充并发。
...
`app.close()` 主动启动 Nest 的终止生命周期并关闭应用连接，测试或受控退出可直接调用；`enableShutdownHooks()` 注册 SIGTERM 等系统信号监听，使进程收到信号时也进入这些钩子。它不是调用 `app.close()` 的前提。两者都不自动保证负载均衡摘流和在途请求排空。
...
应用在收到终止信号后切换 readiness、停止接收队列新任务并跟踪在途工作；Kubernetes、Service、Ingress 或外部负载均衡器负责传播摘流。`preStop`、健康检查周期、终止宽限期和应用 drain 超时必须共同预算。超过上限要安全中止或重投，不能无限等待。
...
从数据库允许连接数扣除管理、迁移和故障余量，再除以服务最大副本数，而不是当前副本数；还要计入 worker、只读库和其他服务。结合查询时长、吞吐和池等待压测调整。自动扩容上限变更时必须重新核算，否则 Pod 增加会先把数据库连接耗尽。

**2. WebSocket 如何滚动发布？**

先让旧 Pod 从 readiness 摘除，停止接受新连接，再在有界窗口内通知客户端重连或等待现有连接结束；会话状态若需跨实例，应放到外部存储或使用适配器广播。负载均衡可能需要 sticky session，但仍要设计断线重连、消息去重和版本兼容，不能假设长连接永不迁移。

**3. 定时任务为何会重复执行？**

每个副本都会初始化相同 Cron Provider，因此同一时刻可能有多个实例执行；滚动发布重叠和进程重启还会增加重复。可改用外部调度器投递到队列，或使用带 fencing token 的 leader / 分布式锁。无论调度层如何，任务仍要以执行 ID 或业务键实现幂等。
...
Queue 只改变任务交付、重试和削峰方式；消费者取到消息后，JavaScript 仍在其事件循环执行，CPU 密集计算会阻塞该消费者的心跳、确认和其他任务，甚至导致消息被误判超时重投。应把计算移到有界 Worker 池、子进程或独立服务，并控制消费并发。

**2. Worker 池大小和线程通信成本如何评估？**

以容器可用 CPU、任务 CPU 占用、内存和延迟目标压测，不机械等于宿主机核数；池过大会争抢 CPU、触发 GC 和上下文切换。测量消息序列化、结构化克隆和 transferable 成本，尽量传递紧凑数据。持续观察队列等待、吞吐、event loop lag 与线程利用率。

**3. 哪类任务需要「Queue + Worker」组合？**

既要求持久化调度、失败重试和削峰，又包含明显 CPU 密集计算的任务适合组合，例如大图处理、压缩加密、PDF 渲染或大型规则计算。Queue 保证任务可恢复交付，Worker 隔离 CPU；消费者还需幂等、超时、资源上限和结果存储，二者都不自动提供业务 exactly-once。
...
Nest Kafka request-response 需要客户端实例订阅 reply topic，并在 `connect()` 前调用 `subscribeToResponseOf()`。reply topic 的 partition 数至少覆盖同时运行的客户端应用实例，否则部分实例分不到回复分区而无法发送请求。还要为扩容和滚动发布重叠预留容量，并监控再均衡影响。

**3. `emit()` 成功返回为何不等于业务已处理？**

`emit()` 是 hot Observable，会立即尝试投递，但返回成功最多说明客户端或 transport 已接受某个发送阶段，不保证 broker 已持久化、消费者已收到，更不保证业务事务提交。可靠性取决于 transport 的确认与重投配置；端到端仍需 eventId、消费者幂等、死信、状态和对账。
...
业务数据与 outbox 记录在同一数据库事务提交，所以只要业务成功，就一定留下待发布记录，避免「写库成功但发消息失败」的窗口。发布器发送成功后若在标记已发布前崩溃，会再次扫描并重发。因此它提供可靠记录而非 exactly-once，消费者必须按 eventId 幂等。

**2. 幂等表自身如何清理？**

保留期至少覆盖消息最大重试、延迟队列、灾难恢复和人工重放窗口；过早删除会让旧消息重新产生副作用。可按完成时间分区，先归档审计所需字段，再批量删除或 TTL 清理。对永久业务唯一约束则不能随意删，需区分消息去重记录与业务事实。

**3. 用户看到处理中超时怎么办？**

「处理中」必须对应可查询的任务或业务状态，并给出预计时间、刷新方式和安全重试入口。前端轮询、SSE 或 WebSocket 超时后只停止等待，不宣告失败；用户可凭 operationId 查询终态。服务端设置超时告警、死信和人工补偿，并保证重复提交复用原任务。
...
不必须。CQRS 的核心是写入 Command 与读取 Query 的模型和职责分离，可以先共享同一数据库、甚至同一进程，只使用不同应用服务和投影。只有读写负载、扩展、存储模型或隔离需求出现证据时才分库；一旦分离存储，就要承担投影延迟、重建和最终一致性。

**2. Domain Event 与 integration event 有何区别？**

Domain Event 表达领域内部已发生事实，可在同一 bounded context 内驱动规则，模型可更贴近领域；integration event 是跨模块或跨服务的稳定公开契约，应在事务提交后通过 outbox 发布，并做版本兼容、脱敏和幂等。不要直接把内部对象序列化后当外部事件。

**3. Saga 失败如何补偿？**

Saga 为每个已完成步骤定义可重试的补偿动作和状态转移，例如退款、释放库存，而不是依赖数据库全局回滚。补偿本身也可能失败，必须幂等、持久化进度、指数退避并进入死信或人工处理。对不可逆动作要调整顺序、预留状态或采用人工确认。
...
检查它是否拥有清晰业务语言和数据所有权，只通过少量公开门面、token 或事件与外部通信，没有跨模块注入 Repository 和改表。用依赖图、架构测试与契约测试证明边界，再模拟把接口换成进程外调用，评估事务、延迟、失败和发布依赖，不能只看目录整齐。

**2. 跨模块查询报表怎么做？**

报表可建立只读投影，由各模块通过事件同步所需字段；低频后台查询也可经授权查询接口聚合。若同库直接 join，应把它限制在只读报表适配器，明确不反向修改所属模块数据，并监控耦合。实时性、查询成本和一致性要求决定使用同步聚合还是异步投影。

**3. 一个数据库是否还能称模块化单体？**

可以，部署和物理数据库共享不等于逻辑边界不存在。关键是每个模块拥有明确表与写入入口，其他模块不能绕过公开接口修改，跨模块约束和事务规则有记录并可测试。共享 PrismaService 或 DataSource 可作为连接基础设施，但不能因此允许任意模块查询所有表。
...
**1. 共享 PrismaService 是否必然破坏边界？**

不必然。它可以只是进程级连接和 Prisma Client 生命周期管理，避免每请求创建客户端；真正破坏边界的是其他模块借它任意访问不属于自己的模型。应在仓储、目录依赖和架构测试层限制数据所有权，公开业务门面而非数据库客户端，并审查跨模块查询。

**2. 内存事件何时足够？**

当生产者与消费者在同一进程，事件只用于非关键解耦，允许进程崩溃时丢失，且无需跨服务重放、审计和独立扩缩容时，内存事件足够。涉及业务可靠通知、跨进程集成或必须最终完成的副作用，应使用事务 outbox 和可靠 broker，不能把 EventBus 当持久化队列。

**3. 如何量化模块耦合？**

持续统计跨模块 import 数、公开 API 面积、循环依赖、跨模块数据库访问、一次需求同时修改的模块数，以及变更失败和团队协作冲突。依赖图可看方向，Git 变更耦合可看共同变化频率。指标用于发现趋势，不以单一阈值拆服务，还要结合业务边界判断。
...
interface 编译后不存在，不能表达运行时校验、序列化、HTTP 状态和错误码；前后端也可能安装不同版本，后端实际 JSON 与声明漂移。应以 OpenAPI 或独立 Schema 作为可验证契约，生成类型与客户端，并通过 E2E 和消费者契约测试确认运行时行为，而不是直接共享后端源码类型。

**2. `null` 与字段缺失如何约定？**

缺失通常表示该字段未返回或更新时保持原值，`null` 表示字段存在但当前明确无值，两者不能混用。OpenAPI 要分别声明 `required` 与 nullable，DTO 校验和序列化保持一致。更新接口尤其要区分未修改、清空和空字符串，并用生成客户端及契约测试覆盖。

**3. OpenAPI 生成代码应否手改？**

不应直接修改生成产物，因为下次生成会覆盖，且人工补丁使契约来源失去唯一性。若生成结果错误，应修正服务端 Schema、生成器配置或模板；业务便利方法放在稳定包装层。CI 应校验重新生成后工作树无差异，并让生成客户端通过类型检查和关键接口测试。
...
每个标签页内部先用 single-flight 合并并发刷新，跨标签页再通过 BroadcastChannel 或带租约的共享状态广播「刷新中、成功、失效」状态，不广播 access 或 refresh token。等待设置上限，持有者崩溃后允许重新竞选。服务端 rotation 仍必须用事务处理真正并发，前端协调不是安全锁。

**2. 401 与业务登录失效码如何配合？**

401 表示当前凭证不可接受，请求层只有在稳定错误码表明 access token 可刷新时才触发一次刷新；账号禁用、refresh 失效或会话撤销等 code 应直接退出。403 不刷新。刷新接口自身排除拦截，原请求最多重放一次，防止网络或权限错误形成无限循环。

**3. 如何把前端错误关联到后端 trace？**

网关或 Nest 为每次请求生成 traceId，并在响应错误 envelope 与安全响应头中返回；前端请求层记录 traceId、release、路由和时间，在错误页或客服复制入口展示，不收集 token 与敏感表单。服务端日志、指标 exemplar 和 trace 使用同一标识，跨消息时继续传播上下文。
...
先锁定 NestJS 11 和相关独立包版本，在官方文档、迁移指南与本地类型声明中搜索准确导出和签名，再写最小 TypeScript 复现并实际构建。不要把博客、旧版本示例或相似框架 API 当证据。涉及适配器和 transport 时，还要核对对应包而非只看核心包。

**2. AI 最容易漏掉哪些并发问题？**

常见遗漏包括 refresh rotation 双写、幂等 key 先查后写、缓存回填竞态、事务外客户端、连接池总预算、队列重复投递、锁租约过期和多副本 Cron。审查时画出并发时序，找原子边界和崩溃窗口，再用唯一约束、事务、屏障并发测试与故障注入验证。

**3. 代码能编译为什么仍不能合并？**

编译只证明静态语法和可见类型成立，不证明模块 token 装配、运行时 DTO 校验、授权、事务原子性、超时取消、资源关闭和生产 transport 行为正确。合并前还需单元、集成与 E2E、题库校验、文档构建、依赖审计，并对高风险路径做并发和故障验证。
