...
**失败形态：** 「项目大」直接等于要上微前端；一边迁微前端一边重写权限/组件库/全部接口，变量过多无法归因。

**验收：** 4～8 周数据证明发布冲突或故障隔离收益；试点失败可退回单体或独立站；平台 Owner 与契约测试到位前不上运行时装配。

仓库多、包多、团队多都不等于微前端：要看是否独立构建部署与故障隔离。为技术栈自由上微前端却让首屏更慢，是典型失败——先量化发布冲突与重复依赖字节，能 Monorepo + 兼容组共享则共享；必须异构则计入体积预算并设性能门禁。
...
分层架构要有依赖方向：页面 → 用例 → 领域 → 基础设施端口；共享包必须语义一致、变化节奏一致且有 Owner——优先复制小而不稳定的代码。有效分层不是目录整齐，而是更换请求库/UI 时核心规则不用重写。万能 `common` 是事故源。稳定内核、平台能力、业务复用三者版本策略不同，不能合成伪精确的「共享得分」。共享包评估要分开「风险暴露」与「治理成本」。
...
JS 沙箱（默认 Proxy）隔离的是可信应用间全局读写冲突，**不是**浏览器安全边界；`document`、Cookie、网络、持久化存储仍可逃逸。样式：优先 Namespace / Token / lint；`strictStyleIsolation`（Shadow DOM）最强但 Portal/主题要适配；`experimentalStyleIsolation` 是实验能力且部分 at-rule 不改写。Shadow **不隔离 JS**。

预加载按下一跳概率分级，不能默认 `prefetch: 'all'` 抢首屏。通信：一次性上下文用 `props`；少量公共状态可用 `initGlobalState`；复杂业务回版本化事件或后端事实源。**不要直接共享 Pinia 实例。** 卸载用资源账本连续 mount/unmount ≥20 次验收监听与堆增量。

**失败形态：** 开启 sandbox 就宣称安全评审完成；全局可变 Store 地狱；卸载泄漏定时器导致后台 CPU 常亮。

**验收：** 连续 mount/unmount ≥20 次监听与 Detached DOM 不持续涨；预加载不抢首屏 LCP；样式隔离模式写进 ADR 并测 Portal/主题。
...
PoC 必测五项：真实旧应用接入改动；首次/二次加载瀑布；样式/全局/动态脚本隔离；并行多实例与重复卸载；入口故障灰度回滚与定位链路。一票否决写进 ADR：CSP/安全边界、深链刷新、可回滚发布、核心 SDK 兼容、内存预算。
...
动态加载（MF 2.0）：受控清单 `registerRemotes` + `loadRemote`；改 URL **不能**假定热替换已执行模块。`remoteEntry` 可执行容器；`mf-manifest.json` 描述入口/资产/shared/类型；类型只保开发期契约。失败按 Manifest → remoteEntry → chunk → init/get → expose → shared → 执行分层；网络可有限重试，执行/契约错误立即熔断并应用级降级，保住 host。覆盖同名 `remoteEntry.js` 无法可靠回滚——清单指针切换并保留整套不可变资产。

打包侧边界链到 [07](/interview/review/topics/07-vite-webpack)：答到内置还是 enhanced、eager、shareKey；运行时协商留在本题。

**失败形态：** 配了 Webpack 内置联邦却按 MF 2.0 文档找 Manifest；remoteEntry 404 无限重试打爆 CDN；把 `shared` 当 externals 期望构建期缺依赖即失败。

**验收：** ADR 写明插件包名与文档链接；host×remote 冒烟含 shared 快照；入口故障按阶段熔断并切 LKG；会话固定 release。
...
模式：`alive` 保活（快、耗内存，改 URL 不自动驱内部路由）；单例；重建。通信优先 props，事件带命名空间/版本；保活应用易收「幽灵事件」——回调检查激活代次。性能分首次/二次/内存/后台 CPU，不能只用「秒开」Demo。

**失败形态：** 因用了 iframe 就宣称跨域安全隔离；保活页改 URL 期望自动驱路由却未配置。

**验收：** 同域互访用例证明非安全边界；低信任改独立 origin；保活/单例/重建决策写进产品约束。

### 7. 隔离梯队、通信、权限、灰度、安全、观测

CSS：Namespace → scoped → Shadow → iframe；JS：自律 → Proxy/VM → 同域 iframe → 跨域 iframe。隔离越强，主题/弹窗/调试成本越高。

通信：共享只读 AuthContext/租户/主题；领域事件版本化；写操作与强一致回后端。初始化用 props；通知用版本化事件。禁止全局可变 Store 地狱。多租户切换要提升会话代次、清空租户键缓存与保活实例，服务端仍校验归属。

登录：统一身份层；前端隐藏按钮不是安全；Token 优先同域 BFF + HttpOnly Cookie；子应用独立运行走同一登录协议。权限服务不可用时：高风险默认拒绝；只读可有签名短 TTL 快照并明示降级，绝不能前端超时就全量放行。

灰度：清单按桶返回不可变 release，会话固定版本；回滚切清单指针并保留整套资产。每季度至少演练一次：人为让某 remote Manifest 404，验证熔断、降级文案、LKG 切换与告警路由是否命中正确 Owner。

安全：远程入口是可执行供应链；qiankun/Garfish 部分链依赖 `eval`/`new Function`，与禁 `'unsafe-eval'` 的 CSP 冲突——高安全域优先独立域 iframe，不为兼容永久放宽 CSP。信任分级：内部可信应用可用 Proxy/VM 防冲突；第三方与高敏感报表必须独立域。

观测：装载分阶段耗时与结果；上下文带 host/app/release/traceId；「可用」= 业务视图可交互，不只是 `mount` resolve。SourceMap 私有；告警对人负责。

### 8. 巨石迁移、四方案选型与系统设计默认路径
...
管理系统设计口述：先壳（会话、布局、动态路由、请求层、错误页）+ 业务包懒加载；权限以后端为准；动态组件清单要验签。微前端是演进选项，不是 MVP 默认。壳用 Webpack、新业务用 Vite 可以，但必须统一清单、身份、观测与错误边界，并由适配层隐藏框架细节——混用不等于两套生命周期重复管理同一边界。Rspack/Vite 上的联邦集成深度以对应文档为准，PoC 前写进矩阵。

组织语言收口：微前端是组织扩展工具，不是技术时尚。没有平台 Owner、没有契约测试、没有回滚演练，拆得越细，事故越大。

完成度指标不是「搬仓百分比」，而是：子应用可独立运行与深链刷新、权限以后端为准、卸载干净、灰度可回滚、观测能按 app/release 归因。试点必须可退。把独立发布占比、装载 P95 与演练 MTTR 带进答辩，比罗列四套 API 更像负责人。
...
- 约束：审批页 alive，用户已切走仍收到全局事件。
- 方案：事件带命名空间与版本；回调检查激活代次；切走暂停订阅或丢弃。
- 取舍：恢复速度 vs 错误处理复杂度。
- 验证：切走后注入事件不得改隐藏页状态。
- 防护：事件目录 + 代次断言进集成测试。

证据坑位（填你的数）：独立发布占比〔填〕；联动发版次数〔填〕；回滚耗时〔填〕；子应用装载 P95〔填〕；重复框架体积〔填〕；重复卸载 20 次堆增量〔填〕；入口故障 MTTR〔填〕。
...
- 「shared 等于 externals 吗？」→ **否；运行时协商，失败形态不同。**
- 「先全量微前端再补治理？」→ **否；先契约与平台能力，再扩大装配。**
