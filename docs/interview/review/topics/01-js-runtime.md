...
口述结构固定：用途 → 抓住的是引用还是值 → 举一个过期闭包事故。`let` 在 `for` 里能「修好」经典题，是因为每次迭代有新的词法绑定，不是魔法复制；若闭包捕获对象，仍共享对象引用。模块顶层变量本身不叫闭包，但模块内导出函数若引用它，就形成对模块词法环境的闭包——模块环境通常长期存活，SPA 路由切换后更容易残留单例状态。排查泄漏要用 Heap Snapshot 的 retaining path 证明「业务生命周期已结束仍可达」，闭包只是路径上的一环；修复应找到 owner，建立注册与注销的对称关系。 事件总线、模块单例、未被 abort 的 fetch 回调，是 SPA 里三类高频泄漏源——口述要能各举一例并说出对称清理点。
...
**验收：** 能解释为何「微任务切片」不能换来渲染；能区分 `nextTick`、rAF、`setTimeout(0)` 各自解决什么；卡顿故事有前后分位数，不只有感觉。
...
**机制：** 防抖重置计时器；节流用时间戳或定时器保证周期上限；`AbortController` 把取消令牌沿调用链下传，`fetch` 在 abort 后仍可能有尾包到达——写回门闩必须看 signal 或代次。leading/trailing 组合决定「第一次立刻跑还是最后一次补跑」，产品语义不同。

**失败形态：** 只防抖不取消，慢请求仍盖新词；卸载未 `cancel`/`abort`，已销毁组件改状态；把 AbortError 当业务失败 toast。

**验收：** 快速输入竞态用例；卸载后无写回；AbortError 静默或可区分文案。
...
**失败形态：** `Promise.all` 一拒就放弃却不取消其余任务，继续占连接；结果不保序导致 UI 对错行；重试对 4xx 盲打放大雪崩；空 `catch` / 拦截器吞错后返回 `undefined` 造成二次类型错误。

**验收：** 飞行中数量 ≤ N；取消后无写回；失败分类可观测（传输 vs 业务）；链尾必有消费拒绝路径。
...
**失败形态：** 用 `JSON.parse(JSON.stringify)` 拷含 Date/Map/循环的对象丢数据或抛错；把 `structuredClone` 当万能深拷贝（函数/DOM 不行）；热路径整树克隆造成卡顿与内存尖峰。

**验收：** 明确「不可变局部更新」协议；克隆边界写进工具函数注释；大对象优先结构共享而非整树拷贝。
...
Worker 边界：可迁纯计算与编解码，不能直接搬 DOM；`postMessage` 有结构化克隆成本，SharedArrayBuffer 有安全与兼容门槛。切片与 Worker 的选择看：是否必须读布局、消息往返是否吃掉收益、取消如何传播。证据用同一设备档位的 Long Task / INP，不拿开发机单次 FPS 交差。

**验收口令：** 能画出「同步 → 微任务 →（可能）渲染 → 宏任务」；能说明微任务递归为何饿死渲染；能设计带 Abort 的竞态用例；能区分框架 `nextTick` 与 rAF。
...
**案例骨架 E — 「模块单例导致路由切走仍推送」**

- 约束：WebSocket / EventBus 挂在模块顶层。
- 方案：按会话或页面代次订阅；离开时对称 unsubscribe；禁止匿名闭包只加不拆。
- 取舍：多一层生命周期管理。
- 验证：进出 20 次连接数与监听数不涨。
- 防护：资源账本 + 卸载单测。
...
- 「微任务递归能让出渲染吗？」→ **不能；会饿死渲染。**
- 「structuredClone 能拷函数吗？」→ **不能。**
