...
>
> **每日时长：** 默认 90～150 分钟。**场次时长计入当日预算。** 终场日（D14）因 75 分钟脚本，允许拉到约 180 分钟；推荐把「上场 Top 5」提前到 D13 晚间，使 D14 以模拟 + 短复盘为主。
>
...
| D13 | 领导力 + 行为 + 主故事（晚间预合并 Top 5） | `r14-d13-lead-story` |
| D14 | 终场模拟 + 短复盘 / 反问 | `r14-d14-final-mock` |
...
> **砍枝优先级：** ① TS 专题 15 分钟主线口述（必做）→ ② `unknown` / 收窄 / 泛型各一例 → ③ 手写入口 25 分钟（**可选**）。超时先砍手写，只留「失败短路与取消」口头一句。

#### 学习目标

- TS 类型系统专题可讲 15 分钟
- 与手写 / 工程约束的边界说清（手写可降级为口述边界）
...
- **可选（预算有余）：** 选 1 道手写入口题限时 25 分钟（并发池或 `Promise.all` 思路即可），讲失败短路与取消；超时则改为口头讲边界，不占深挖
...
> **砍枝优先级：** ① 主故事脱稿 + 领导力速记 → ② 行为专题主线 → ③ 追问树 L2（可选）。**晚间必做：** 合并两周卡点为「上场 Top 5」并各口述一遍，减轻 D14 负担。

#### 学习目标

- 领导力速记 + 行为专题
- 主故事压实到可上场
- 预合并上场 Top 5，供明日终场后只做短复盘
...
- 两周卡点笔记（速记 / 专题回链见 [复习总览](/interview/review/00-overview)）

#### 必做输出

- [09 领导力速记](/interview/review/sheets/09-leadership) 遮稿 5～8 分钟
- [12 行为专题](/interview/review/topics/12-lead-behavioral) 15 分钟主线；追问树：预算够则走至少 2 条到 L2，超时砍到 1 条主问
- 产出 5 分钟 STAR + 90 秒电梯版；录音第二遍只看提纲；失败 / 取舍分支各 90 秒
- 简历证据与故事数字勾对清单；准备明日终场开场稿
- **晚间：** 合并两周卡点为「上场 Top 5」，每条遮稿口述勾选通过 / 仍卡（仍卡者只标回炉速记结论卡，不新开专题）

#### 验收标准

主故事脱稿；行为追问有收口；上场 Top 5 清单已就绪。
...
> **时长安排：** 默认 **脚本十一（75 分钟）+ 评分短复盘 + 反问勾选**，场次计入当日预算；本日记为终场日，允许合计约 **150～180 分钟**。若 D13 晚间已完成 Top 5，则 D14 **不再重跑**整表口述，只对「仍卡」条目做 ≤15 分钟补讲。若 Top 5 未提前做完，可并入本日但优先保证模拟完整，合计可到约 180 分钟。
...
- [模拟脚本 · 脚本十一 · 综合终场](/interview/mocks/scripts#脚本十一-综合终场-75-分钟)（或等价完整场：脚本六 / 三，按岗位；场次时长计入预算）
- [评分表](/interview/mocks/scorecard)
- [反问清单](/interview/mocks/reverse-questions)
- D13 已备好的上场 Top 5 与回炉标注

#### 必做输出

- 完整跑完 **脚本十一**（或岗位等价终场）；用评分表覆盖目标岗位核心维（复盘限 10～15 分钟）
- 确认 D13 Top 5：已通过的勾掉；仍卡者各口述一遍（≤15 分钟总预算），只回对应速记结论卡
- 从 [反问清单](/interview/mocks/reverse-questions) 定 3～5 条上场版（与模拟反问环节可合并，不重复两轮长讨论）
- 禁止新开专题与通宵刷题；确认睡眠与上场节奏

#### 验收标准

终场模拟完成；上场反问版已定；最高频卡点口述通过（含 D13 预检）。
