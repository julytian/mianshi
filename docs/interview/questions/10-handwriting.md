...
策略：每次调用创建 Promise，清除旧定时器，并主动以自定义 `CanceledError` 拒绝旧调用；最后一次到期后以最新参数和 `this` 执行，采用 `Promise.resolve` 吸收同步返回与异常。复杂度：每次 O(1)，空间与待拒绝调用数相关；及时结算后回收。边界：处理同步抛错、取消和组件销毁。测试：连发三次只执行一次，前两次拒绝，末次值和异常均正确透传。
...
策略：以源对象为键记录副本，在递归子属性前先写入映射，重复遇到同一对象时直接返回副本，从而保留共享引用并打断环。复杂度：访问 n 个属性通常时间 O(n)、空间 O(n)。边界：WeakMap 只接受对象键且不可枚举，正好避免克隆结束后映射长期强引用源图。测试：覆盖自环、互环和两个属性指向同一对象，断言不爆栈且副本引用关系一致。
...
策略：把静态数组改成可关闭队列，`add` 入队并唤醒等待 worker，worker 无任务时等待条件 Promise，队列关闭且在途归零后整体完成；索引在入队时分配以保持结果顺序。复杂度：入队、出队摊还 O(1)，空间 O(q+n)。边界：明确关闭后拒绝追加、空队列等待、生产者异常和取消时唤醒全部 worker。测试：先启动空池再分批追加，验证并发上限、顺序、close 后结束及取消不挂起。
...
策略：泄漏通常来自长生命周期总线强引用组件闭包、`once` 永未触发、匿名函数无法 off，以及事件键对应空 Set 未删除；让 `on` 返回取消函数，并在解绑后清理空集合。复杂度：注册和删除平均 O(1)，总空间 O(监听器数)。边界：同一函数重复注册时 Set 会去重，业务是否允许重复需约定。测试：反复挂载卸载后检查监听数量归零，用 WeakRef 或堆快照辅助确认组件可回收。
...
策略：`typeof` 区分原始类型和 function，但 null 得到 object；`instanceof` 依据 `Symbol.hasInstance` 或 prototype 判断对象关系。复杂度：`typeof` 时间 O(1)，`instanceof` 时间 O(h)，空间均 O(1)。边界：跨 realm、原型变更和自定义钩子会影响结果。测试：覆盖 primitive、null、函数、继承、自定义钩子及 iframe 对象。
...
策略：建立 permission 到叶节点的反向索引，并缓存节点的可见子计数；权限变化只重算受影响叶及其祖先，用角色摘要隔离缓存。复杂度：受影响节点 a、树高 h 时约 O(a·h)，索引空间 O(n)。边界：目录权限和批量更新要合并，变化过多时全量 O(n) 更合算。测试：随机切换权限，把增量结果与每次全量过滤做性质对比。
...
策略：hover 只是语言服务的展示，可能被简化、截断或受缓存影响，不能证明错误输入一定拒绝；应把正例赋值和反例 `@ts-expect-error` 固化进源码，由项目锁定版本的 `tsc --noEmit` 校验。复杂度：单个小用例可近似常量，整体编译成本随类型图增长。边界：必须避免宽泛 any 让测试虚假通过，并确保 expect-error 确实命中错误。测试：故意移除约束，确认反例会因“未产生错误”而失败。
...
策略：使用异步阻塞队列；worker 无任务时 await 通知 Promise，`add` 唤醒等待者，`close` 或 abort 唤醒全部并结束，禁止空转。复杂度：入队、领取摊还 O(1)，空间 O(q+limit)。边界：fail-fast 后禁止 add 和领取；重试仍占当前 worker，AbortSignal 同时取消等待与在途任务。测试：覆盖空池后追加、worker 竞争、关闭空队列、失败后拒绝追加及取消无悬挂。
